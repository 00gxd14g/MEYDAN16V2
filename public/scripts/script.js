document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape' && navOverlay && navOverlay.classList.contains('active')) {
            closeNav();
        }
    });
    ...
    const toggleFloatingButton = () => {
        ...
        floatingBtn.classList.toggle('visible', window.scrollY > heroBottom);
    };
    ...
    if (floatingBtn) {
        floatingBtn.addEventListener('click', () => smoothScrollTo('#iletisim'));
    }

    /* Advantages slider with navigation */
    ...
    const prevBtn = advantagesSlider?.querySelector('.slider-nav.prev');
    const nextBtn = advantagesSlider?.querySelector('.slider-nav.next');

    if (advantagesTrack && prevBtn && nextBtn) {
        ...
        const advantageToggles = advantagesSlider.querySelectorAll('.advantage-toggle');
        advantageToggles.forEach((toggle) => {
            toggle.addEventListener('click', () => {
                const expanded = toggle.getAttribute('aria-expanded') === 'true';
                const details = toggle.closest('.advantage-card')?.querySelector('.advantage-details');
                const toggleText = toggle.querySelector('.advantage-toggle-text');
                ...
                if (toggleText) {
                    toggleText.textContent = expanded ? 'Detayları Göster' : 'Detayları Gizle';
                }
